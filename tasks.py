...
class FinancialTasks:
    ...
    def create_document_extraction_task(self, agent, extracted_docs: Dict[str, str], company_name: str) -> Task:
        """Create task for extracting and organizing financial data"""
        
        docs_context = self._format_documents(extracted_docs)
        
        return Task(
            description=f"""Analyze the following financial documents for {company_name} and extract 
            ...
            agent=agent,
            expected_output="""Comprehensive structured financial data extraction organized by:
    ...
    def create_business_analysis_task(self, agent, extracted_docs: Dict[str, str], company_name: str) -> Task:
        """Create task for analyzing business model"""
        
        docs_context = self._format_documents(extracted_docs)
        
        return Task(
            description=f"""Based on the financial documents and any business information 
            contained within them, provide a comprehensive analysis of {company_name}'s 
            ...
            agent=agent,
            expected_output="""Clear, comprehensive business model analysis including:
    ...
    def create_growth_analysis_task(self, agent, extracted_docs: Dict[str, str], company_name: str) -> Task:
        """Create task for analyzing growth metrics"""
        
        docs_context = self._format_documents(extracted_docs)
        
        return Task(
            description=f"""Conduct a thorough analysis of {company_name}'s growth 
            trajectory, key performance indicators, and pricing power.
            ...
            agent=agent,
            expected_output="""Detailed growth and KPI analysis including:
    ...
    def create_valuation_task(self, agent, extracted_docs: Dict[str, str], 
                            valuation_params: str, company_name: str) -> Task:
        """Create task for company valuation"""
        
        docs_context = self._format_documents(extracted_docs)
        
        return Task(
            description=f"""Perform a comprehensive valuation analysis of {company_name} 
            using the provided valuation parameters and methodologies.
            ...
            agent=agent,
            expected_output="""Comprehensive valuation analysis with:
    ...
    def create_investment_recommendation_task(self, agent, company_name: str, 
                                             task_context: List[Task]) -> Task:
        """Create final investment recommendation task"""
        
        return Task(
            description=f"""As a Senior Investment Advisor, synthesize all previous 
            ...
            agent=agent,
            expected_output="""Clear, actionable investment recommendation including:
            ...
            context=task_context  # This task depends on all previous tasks
        )
    
    def create_tasks(self, agents: List, extracted_docs: Dict[str, str], 
                    valuation_params: str, company_name: str) -> List[Task]:
        ...
        task_recommendation = self.create_investment_recommendation_task(
            agent=agents[4],
            company_name=company_name,
            task_context=[task_extract, task_business, task_growth, task_valuation]
        )
